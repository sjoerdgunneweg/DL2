...
#---------our additions-------------------------------------------------------
def get_image_path(folder, i):
    default = f"{i}.png"
    with_suffix = f"{i}_mast3r.png"

    return with_suffix if exists(join(folder, with_suffix)) else default
#----------------------------------------------------------------------------
...
def get_stored_demos(amount: int, image_paths: bool, dataset_root: str,
    ...
    for example in selected_examples:
        ...
        for i in range(num_steps):
            ...
            # si = IMAGE_FORMAT % i
            si = get_image_path(l_sh_rgb_f, i)
            si_original = f"{i}.png"
            if obs_config.left_shoulder_camera.rgb:
                obs[i].left_shoulder_rgb = join(l_sh_rgb_f, si_original)
            ...
            if obs_config.left_shoulder_camera.mask:
                obs[i].left_shoulder_mask = join(l_sh_mask_f, si_original)
            if obs_config.right_shoulder_camera.rgb:
                obs[i].right_shoulder_rgb = join(r_sh_rgb_f, si_original)
            ...
            if obs_config.right_shoulder_camera.mask:
                obs[i].right_shoulder_mask = join(r_sh_mask_f, si_original)
            if obs_config.overhead_camera.rgb:
                obs[i].overhead_rgb = join(oh_rgb_f, si_original)
            ...
            if obs_config.overhead_camera.mask:
                obs[i].overhead_mask = join(oh_mask_f, si_original)
            if obs_config.wrist_camera.rgb:
                obs[i].wrist_rgb = join(wrist_rgb_f, si_original)
            ...
            if obs_config.wrist_camera.mask:
                obs[i].wrist_mask = join(wrist_mask_f, si_original)
            if obs_config.front_camera.rgb:
                obs[i].front_rgb = join(front_rgb_f, si_original)
            ...
            if obs_config.front_camera.mask:
                obs[i].front_mask = join(front_mask_f, si_original)
